...
import { normalizeCodexInternalLogEvent } from "./normalizeCodexInternalLogEvent";

const sourcePath =
  "/Users/demo/.codex-internal/sessions/2026/04/02/rollout-2026-04-02T10-46-14-019d4c15-8d42-78f1-955e-d57f67061b9e.jsonl";

describe("normalizeCodexInternalLogEvent", () => {
  it("maps session_meta to a running Codex Internal session event", () => {
    const event = normalizeCodexInternalLogEvent(
      JSON.stringify({
    ...
    expect(event).toMatchObject({
      ...
      tool: "codex-internal",
      status: "running",
      task: "Codex Internal session: codepal",
      meta: {
  ...
  it("maps user_message events to running activity", () => {
    const event = normalizeCodexInternalLogEvent(
      JSON.stringify({
        ...
        payload: {
          type: "user_message",
          message: "继续推进 Codex Internal 接入。",
        },
      }),
      sourcePath,
    );
    ...
    expect(event).toMatchObject({
      ...
      tool: "codex-internal",
      status: "running",
      task: "继续推进 Codex Internal 接入。",
      activityItems: [
        {
          kind: "message",
          source: "user",
          title: "User",
          body: "继续推进 Codex Internal 接入。",
        },
      ],
      meta: {
  ...
  it("maps final_answer agent messages to completed status", () => {
    const event = normalizeCodexInternalLogEvent(
      JSON.stringify({
  ...
  it("maps turn_aborted to idle status", () => {
    const event = normalizeCodexInternalLogEvent(
      JSON.stringify({
  ...
  it("maps context_compacted to idle status", () => {
    const event = normalizeCodexInternalLogEvent(
      JSON.stringify({
        timestamp: "2026-04-02T02:46:58.283Z",
        type: "event_msg",
  ...
  it("returns null for invalid JSON", () => {
    const event = normalizeCodexInternalLogEvent("{bad json", sourcePath);
    expect(event).toBeNull();
  });

  it("falls back to sessionId from filename when entry lacks payload.id", () => {
    const event = normalizeCodexInternalLogEvent(
      JSON.stringify({
        timestamp: "2026-04-02T02:46:49.900Z",
        type: "session_meta",
