...
export function buildSupportDiagnosticsReport(input: SupportDiagnosticsInput): string {
  ...
  lines.push("");
  lines.push("History");
  lines.push(`Enabled: ${input.historyDiagnostics?.enabled ? "yes" : "no"}`);
  lines.push(`DB Size Bytes: ${input.historyDiagnostics?.dbSizeBytes ?? 0}`);
  lines.push(`Session Count: ${input.historyDiagnostics?.estimatedSessionCount ?? 0}`);
  lines.push(`Activity Count: ${input.historyDiagnostics?.estimatedActivityCount ?? 0}`);
  lines.push(`Last Cleanup At: ${formatDateTime(input.historyDiagnostics?.lastCleanupAt)}`);
