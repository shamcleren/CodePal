...
import { normalizeCodexInternalLogEvent } from "../../adapters/codex-internal/normalizeCodexInternalLogEvent";
...
type CodexInternalSessionWatcherOptions = {
  sessionsRoot: string;
...
export function createCodexInternalSessionWatcher(options: CodexInternalSessionWatcherOptions) {
  const pollIntervalMs = options.pollIntervalMs ?? 1500;
  const initialBootstrapLookbackMs =
    options.initialBootstrapLookbackMs ?? ACTIVE_SESSION_STALENESS_MS;
  const cursors = new Map<string, FileCursor>();
  const debug = process.env.CODEPAL_DEBUG_CODEX_INTERNAL === "1";
  const scheduler = createAdaptivePollScheduler({
    poll: () => api.pollOnce(),
    fastIntervalMs: pollIntervalMs,
    maxIntervalMs: pollIntervalMs * 4,
    onError: (error) => {
      console.error("[CodePal Codex-Internal] poll failed:", (error as Error).message);
    },
  });
  ...
  async function pollFile(filePath: string, initialCutoffMs: number | null): Promise<boolean> {
    ...
    for (const line of lines) {
      ...
      const normalized = normalizeCodexInternalLogEvent(trimmed, filePath);
      if (!normalized || !isSessionStatus(normalized.status)) continue;
      if (debug) {
        console.log(
          "[CodePal Codex-Internal] event",
  ...
  const api = {
    async pollOnce() {
      ...
      if (debug) {
        console.log("[CodePal Codex-Internal] polling root", options.sessionsRoot, "files", files);
      }
