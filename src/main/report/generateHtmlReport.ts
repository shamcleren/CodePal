...
export function generateHtmlReport(input: HtmlReportInput): string {
  ...
  for (const m of input.byModel) {
    ...
    const modelDisplayName = input.redaction?.redactModelNames ? "model" : m.model;
    modelRows += `<tr>
  ...
  for (const session of input.topSessions ?? []) {
    ...
    const modelLabel = redactModels ? "model" : session.model;
    topSessionRows += `<tr>
      <td>
...
<div class="container">
  ...
  ${input.redaction?.redactSessionTitles || input.redaction?.redactModelNames
    ? `<p class="footer" style="color:#e2a03f">Redacted: ${[
        input.redaction?.redactSessionTitles ? "session titles" : "",
        input.redaction?.redactModelNames ? "model names" : "",
