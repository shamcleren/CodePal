...
import { normalizeClaudeInternalLogEvent } from "../../adapters/claude-internal/normalizeClaudeInternalLogEvent";
...
type ClaudeInternalSessionWatcherOptions = {
  projectsRoot: string;
...
export function createClaudeInternalSessionWatcher(options: ClaudeInternalSessionWatcherOptions) {
  ...
  const scheduler = createAdaptivePollScheduler({
    ...
    onError: (error) => {
      console.error("[CodePal Claude-Internal] poll failed:", (error as Error).message);
    },
  });
  ...
  async function pollFile(filePath: string, initialCutoffMs: number | null): Promise<boolean> {
    ...
    for (const line of lines) {
      ...
      const normalized = normalizeClaudeInternalLogEvent(trimmed, filePath);
