...
- Release workflow validates macOS updater assets, including `latest-mac.yml`, dmg / zip artifacts, blockmap files, zip-extracted app signing, and dmg-mounted app signing
...
- v1.0.3 through v1.3.17 are all shipped. Current shipped baseline is **v1.3.17**.
...
- v1.3.17 shipped: model pricing history modal polish, non-current pricing event filtering, and test alignment for vendor filters / Provider Gateway health checks
...
- v1.3.8 local validation on 2026-06-07 covers lifecycle alignment, disabled native-session reply paths, ACP Sessions documentation, and release version bump:
  ...
  - `npm run lint`
  - `npm run build`
- v1.3.9 local validation on 2026-06-08 covers the macOS updater packaging hotfix, release asset validation hardening, and Daily Trend Agent / Model grouping:
  - `npm test -- src/main/releaseBuild/afterAllArtifactBuild.test.ts`
  - `npm test -- src/renderer/components/AnalyticsLineChart.test.tsx src/renderer/components/AnalyticsPage.test.ts src/renderer/projectViewPreferences.test.ts src/main/releaseBuild/afterAllArtifactBuild.test.ts`
  - `npm run lint`
  - `npm test`
  - `npm run build`
  - `git diff --check`
...
- v1.3.13 local validation on 2026-06-09 covers CodeBuddy quota fetching/display, refresh configuration, available-percentage usage strip wording, and the carried cross-agent tool-call label cleanup:
  ...
  - `npm run build`
  - `git diff --check`
- v1.3.14 local validation on 2026-06-09 covers the macOS updater install handoff after the update package has downloaded but ShipIt remains pending under launchd:
...
- v1.3.16 local validation on 2026-06-09 covers CodeBuddy login-window detachment, stable session project attribution, and stale ShipIt pending-cache handling:
  ...
  - `npm run test:e2e`
  - `git diff --check`
- v1.3.17 local validation on 2026-06-10 covers model pricing history modal polish, non-current pricing event filtering, Analytics vendor-filter preference alignment, and Provider Gateway health-check dedupe expectations:
  - `npm test -- src/main/history/historyStore.test.ts -t "omits non-current models from pricing change events"`
  - `npm test -- src/main/history/historyStore.test.ts src/shared/pricingHistory.test.ts src/renderer/components/ModelPricingTrendChart.test.tsx`
  - `npm test -- src/renderer/components/AnalyticsPage.test.ts src/renderer/components/ModelPricingTrendChart.test.tsx`
  - `npm test -- src/renderer/projectViewPreferences.test.ts src/main/gateway/claudeDesktopGateway.test.ts`
  - `npm test`
  - `npm run lint`
  - `npm run build`
  - `npm run test:e2e`
...
- Daily Work Review is available as a personal memory surface over current and persisted sessions. It groups recent work by day and project, separates completed and ongoing items, labels managed / observed sources, and keeps current-session jump affordances available where possible.
- Project attribution is shared across session history, usage backfill, session list grouping, Analytics filters, notifications, and Work Review entries so repository-level scans no longer depend on a single renderer-only grouping path.
- Session rows now expose running duration, latest running duration, total session duration, and context pressure when source data supports those signals.
...
### v1.1.0-v1.3.17 Release Track

v1.1.0 through v1.3.17 are shipped. See individual release notes for details:
...
- `docs/release/notes/release-notes-v1.3.17.md` — model pricing history modal polish, non-current pricing event filtering, and vendor-filter / Gateway health-check test alignment
