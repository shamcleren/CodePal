...
v1.1.0–v1.1.5 已全部交付，v1.1.6 作为当前 patch candidate 准备发版后，近期重点转向：

### Tier 2 Agent / 终端扩展

- **Gemini CLI**：hook event 结构不同（`SessionStart` / `BeforeAgent` / `AfterAgent` / `Notification`），需要写 `geminiHook.ts`
- **Kimi CLI**：payload 跟 Claude 几乎一样，但配置在 `~/.kimi/config.toml` `[[hooks]]`，需要 TOML 安装器
- **Warp**：环境变量（`$WARP_IS_LOCAL_SHELL_SESSION`）已在 hook 层捕获，但 jump 和 send-message 未实现；Open Island 的方案（SQLite pane 表 + AX 菜单循环）较复杂，需单独评估
...
- 在 v1.1.6 Codex 子执行归并之后，继续压低 activity timeline 噪音
...
- release notes 保持短，适合应用内更新面板；完整 changelog 细节放到 status / handoff 文档
...
这些方向现在值得先记下来，但仍然应该明确排在 Tier 2 扩展和当前监控基线工作之后。
...
1. 扩展 Tier 2 agent / 终端覆盖（Gemini、Kimi、Warp 评估）
2. 继续做深监控能力
3. 保持 macOS 分发可信度和发布体验稳定
4. 验证持续使用模式
